...
@testset "Ocean boundary flux forms unconsolidated ice without column conduction" begin
    grid = RectilinearGrid(size = (1, 1, 2),
                           x = (0, 1),
                           y = (0, 1),
                           z = MutableVerticalDiscretization((0, 1)),
    ...
    thermodynamics = prescribed_salinity_enthalpy_thermodynamics(
        ...
        boundary_conditions = ColumnBoundaryConditions(
            top = MeltingLimitedSurfaceFlux(flux = 0.0),
            bottom = PrescribedEnergyFlux(flux = 20.0)),
    )
    # This allocated gradient would create a large conductive residual if it
    # were interpreted as an ice column in open water.
    set!(thermodynamics;
         bulk_salinity = 0.0,
         temperature = (x, y, z) -> -1 - 8z)
